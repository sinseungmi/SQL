...
select to_char(hire_date,'YYYY/MM/DD/DAY') as 입사년도, avg(salary) as 평균월급 from employees inner join jobs using(job_id) 
...
select city as 도시, avg(salary) as 평균급여, avg(salary * 12 * (1 + nvl(commission_pct, 0))) as 평균연봉, count(*) as "도시별 직원 수"
from departments d
...
--무조건 메인테이블이 먼저 나오고, 후속 테이블이 뒤에 나와야 한다. 
